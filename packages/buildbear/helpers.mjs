...
async function waitForLiveNode(node) {
  const spinner = ora("Waiting for the Sandbox to be live").start();
  const config = {
    method: "get",
    url: `${BB_BACKEND_URL}/v1/buildbear-sandbox/${node.sandboxId}`,
    headers: {
      Authorization: `Bearer ${BB_API_KEY}`,
      "Content-Type": "application/json",
    },
  ...
  async function checkLoop() {
    try {
      ...
      if (data.status === "live") {
        spinner.succeed("Sandbox is live");
        console.log(node);
      } else if (data.status === "started") {
        await timeout(2000);
        await checkLoop();
      } else {
        spinner.fail("Sandbox has stopped");
      }
